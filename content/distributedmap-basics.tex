...
\section{Custom Keys}
In most cases when you work with a Hazelcast map you probably will make use of a Long, Integer or String as key. But in some cases you will need to create custom keys, but to do it correctly in Hazelcast, you need to understand how this works. 

When you store a key/value in a Hazelcast map, the actual objects are not stored in the map because an object can't move from one JVM to another. Instead the key/value are serialized to byte-arrays and these will be stored in a local ConcurrentHashMap. For keys the hash/equals is used of the byte array version of the object, so equal keys need to result in equal byte arrays. For values the hash/equals of the object is used.
...
\end{lstlisting} 
Then 'key1.equals(key2)' and 'key1.hashCode()==key2.hashCode()' is true. But since the byte-array content of both objects is different, this key implementation should not be used with a Hazelcast map. Although this BrokenKey is a bad candidate to used as a key in a Hazelcast map, it can be used without a problem as a value, since its equals is implemented correctly.
