...
\section{Network Join mechanisms}
Hazelcast currently supports 3 mechanisms for Hazelcast nodes to discover each other and to form a cluster. Without this discovery mechanism, the node's will not form a cluster together, but will remain 1 node clusters.
\begin{enumerate}
\item Multicast:
\item TCP/IP Cluster:
\item Amazon EC2:
...
TODO: Is there also some kind of multicast subnet? How can you control the 'network' where the multicast is 'happening'.
...
<hazelcast>
    ...
    <network>
       <join>
           <multicast enabled="false">
              todo: why are group and port set?
              <multicast-group>224.2.2.3</multicast-group>
              <multicast-port>54327</multicast-port>
           </multicast>
