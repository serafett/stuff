...
[todo: recheck default poolsizes.]
...
[todo: explain the IExecutorService]
...
\section{Routing}
Till so far we didn't care about which member did the actual processing of the task; as long as a member picks it up. But in some cases you want to have control on which member is going to execute. Luckily the Hazelcast IExecutorService provides different ways to route messages:
\begin{enumerate}
...
\end{enumerate}
In the previous section we already covered routing to any member. In the following sections I'll explain the last 3 routing strategies. This is also where a big difference is visible between Hazelcast 2.x and 3, while 2.x relied on the DistributedTask, 3.x relies on explicit routing methods on the IExecutorService.
