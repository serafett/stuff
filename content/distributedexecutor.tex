...
\end{lstlisting}
The PartitionedTask is implementing various interfaces. The most important one in this case is the PartitionAware interface that exposes the 'getPartitionKey' method. This indicates to Hazelcast that when this task is executed, that ghet getPartitionKey method should be called to determine the actual node the tasks is going to be executed. If null is returned as partitionKey, this indicates to Hazelcast that any node will do.
