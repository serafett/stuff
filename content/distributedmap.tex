...
In this chapter, you'll learn how to use one of the most versatile data structures in Hazelcast; the com.hazelcast.core.IMap. The IMap extends the java.util.concurrent.ConcurrentMap interface, and therefor it also extends the java.util.Map. The big difference between normal Map implementation and the Hazelcast IMap implementation is that the later is a distributed data-structure.

Internally Hazelcast divides the map in partitions, and distributes the partitions evenly among the members in the cluster. The partition a map entry is stored on is based on the key of the map entry: each key belongs to a single partition. By default Hazelcast will use 271 partitions for all partitioned data-structures. The value can be changed with the 'hazelcast.map.partition.count' property.

When a new member is added, this member will take its share in the load. The oldest member in the cluster decides which partitions need to be moved to the new member. In Hazelcast 2.x the smallest partitions were moved to prevent network traffic. But with Hazelcast 3.0 the size of the partition isn't determined and therefor will not be part of the criteria to move a partition. Hazelcast can simply scale up by adding new cluster members. It is important to understand that at runtime Hazelcast will move partitions around based on the load: so number of requests, number of map entries, the memory usage of the partition. Hazelcast assumes that the load will be equally spread among the partitions.

When a member is removed, all the partitions it owns, will be moved to other members in the cluster. So scaling down a cluster is simple, just remove members from the cluster. Apart from a 'soft' removal of the cluster, it can also happen that there is a 'hard' removal; e.g. the member crashes or it gets disconnected from the cluster due to network issues. Luckily Hazelcast provides various degrees of failover to deal with this situation. By default there will be one synchronous backup; so the failure of a single member will not lead to loss of data in the IMap.

If there are a huge number of map entries, it can lead to a considerable performance overhead. This is caused by the  garbage collector since the map entries are stored in the Java Heap. To reduce garbage collection times Hazelcast introduced Elestic Memory which is part of the commercial offering.

[todo: is this with elastic memory or just normal ]
On Youtube there is demo: http://www.youtube.com/watch?v=TOhbhKqJpvw where 4 Terabyte of data from 1 billion entries is stored on 100 Amazon EC2 instances, supporting to 1.3 million of operations/second.
...
To demonstrate this basic behavior, the following Member creates a Map and writes some entries in that map:
...
employees.put(123,e);
\end{lstlisting}
Of course, you also need to watch out for data races: see [concurrency control].
...
If you have been using Hazelcast 2.x you might remember the cache-value property. This property has been dropped and replaced by the recordType property.
...
[todo: large keys/values: streams not available]
