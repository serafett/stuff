...
\begin{enumerate}
\item Condition is not supported. With the java.util.concurrent.locks.Lock it is possible to create a java.util.concurrent.locks.Condition object to wait for a certain condition to happen; e.g. a queue just received an element another thread is waiting for.
...
If we first start a leader and then one or more followers. The followers will wait till the leader completes. This example show a ICountDownLatch with only a single 'step'. But if a process has n steps, initialize the CountdownLatch with n instead of 1 and for each completed step call the countDown method.
...
If you need to deal with dynamically created created ICountDownLatches and therefor you don't know the names up front, you can pass the name of the latch instead of the actual object. The actual object can be retrieved usinghzInstance.getCountdownLatch(..).
