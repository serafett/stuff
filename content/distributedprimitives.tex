...
There is 1 difference, with the normal Java version, you can create a condition using the Lock.newCondition() method. Unfortunately this doesn't work in a distributed environment since Hazelcast has no way of knowing if Conditions created on different members are the same condition or not. That is why a condition needs to be created using the ICondition.newCondition(String name) method.

In the following example we are going to create a member that waits for a counter to have a certain value. And another that will set the value. So lets get started with the waiting member:
\begin{lstlisting}[language=java]
...
\end{lstlisting}
As you can see the lock is acquired first and then in a loop the condition is counter is checked. As long as the counter is not 1, the waiter will wait on the isOneCondition. Once the isOneCondition.await() method is called, Hazelcast will automatically release the lock so that a different thread can acquire it. Once the isOneCondition is signaled, Hazelcast will automatically reacquire the lock. This is exactly the same behavior as the JDK provides.

If the WaitingMember is started, it will output:
\begin{lstlisting}
Waiting 
\end{lstlisting}
