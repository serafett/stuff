...
Hazelcast supports various logging mechanisms; 'jdk', 'log4', 'sl4j' or 'none' if you don't want to have any logging. The default is the logging is 'jdk': the logging that is part of the JRE. Logging can be set by using adding a property in the hazelcast configuration:
\begin{lstlisting}[language=xml]
<hazelcast>
    <properties>
...
\end{lstlisting}
But it can also be configured from the commandline 'java -Dhazelcast.logging.type=log4j'. If you are going to make use of 'log4j' or 'slf4j', make sure that the correct dependencies are included. [todo:does commandline override xml-properties/programmatic configuration?]
