...
Hazelcast currently supports 3 mechanisms for Hazelcast members to discover each other and join the cluster. Without this discovery mechanisms, the members's will not form a cluster, but will remain stand alone Hazelcast instances. The discovery mechanisms are:
\begin{enumerate}
\item Multicast:
\item TCP/IP Cluster:
\item Amazon EC2:
...
In a production environment often you want to prevent that the communication between Hazelcast memebers can be tampered or can be read, with because it could contains sensitive information. Luckily Hazelcast provides a solution for that by enabling SSL encryption.
...
\section{Encryption}
Hazelcast only supports Symmetric encryption
