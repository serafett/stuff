...
\section{Member discovery}
Hazelcast supports 3 mechanisms for nodes to discover the cluster: multicast, tcp/ip-cluster, Amazon EC2. Without this discovery mechanisms, the members's will not form a cluster, but will remain stand alone Hazelcast instances. Once the nodes have discovered the cluster, Hazelcast relies on TCP for internal communication.

\subsection{Multicast}
With multicast discovery a member will send a message to all members that listen on the multi-cast port. It is the easiest configuration for testing. But in a lot of production environments, multicast is not supported or desirable.
...
It can be configured through setting interface, or using the hostname.
