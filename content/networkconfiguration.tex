...
\end{lstlisting}		
After this point, we now have a 2 member Hazelcast cluster running on a single machine. Try creating another few members. And you can of course also try to connect multiple members over a network.

If you don't see members joining, then it is likely because multicast has been disabled. If you are on a *NIX environment, you can try to execute the command 'ifconfig' and look for 'MULTICAST'.
...
\item multicast-timeout-seconds:defaults:2
\item trusted-interfaces:
\end{enumerate}

\subsection{TCP/IP cluster}
In the previous section we made use of Multicast, but this is not always an option, e.g. when making use of cloud environments. That is why Hazelcast supports another discovery mechanism: the TCP/IP cluster. The idea behind this approach is that there should be a one or more well known members to connect to. Once new members have connected to the cluster, they will keep each other up to date with all member addresses.

Underneath there is an example of a TCP/IP cluster configuration where such a well known member is configured using its interface:
\begin{lstlisting}[language=xml]
<hazelcast>
    <network>
        <join>
...
\end{lstlisting}
If you need to create a range of members the following syntax can be used: '192.168.1.0-7' or by a comma separated list or by adding multiple '<interface>..</interface>' entries. Well known addresses can be configured through setting interface, or using the hostname. [todo: also port can be included]


For the none-multicast option, all or subset of cluster members' hostnames and/or ip addresses must be listed. Note that all of the cluster members don't have to be listed there but at least one of them has to be active in cluster when a new member joins. The tcp-ip tag accepts an attribute called "conn-timeout-seconds". The default value is 5. Increasing this value is recommended if you have many IP's listed and members can not properly build up the cluster.

[todo]
By removing or setting to a higher value, 'hazelcast.merge.first.run.delay.seconds' and 'hazelcast.merge.next.run.delay.seconds' properties. On this setup, Hazelcast searches for other members every 5 seconds.For TCP-IP join strategy, Hazelcast tries to connect at max three successive ports for an address. Since your first node's address is bound to 127.0.0.1:5701, it only tries for remaining two ports (5702 and 5703).
[todo]
...
There is currently no support for encryption between a native client and a cluster member; is there an alternative?
