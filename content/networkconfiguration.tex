...
\end{lstlisting}
As you can see, the port is 5701. If another member is started, you will see that port 5702 is assigned. If you look carefully at the end of the logging, you'll see a warning about cluster size is 1. This is because the created members are currently not part of a cluster, but are standalone. To create a cluster, see the next chapter.
...
With multicast discovery a member will send a message to all members that listen on the multi-cast group. It is the easiest mechanism to configure, but is not always available. Underneath you can see a very minimalistic multicast configuration:
\begin{lstlisting}[language=xml]
<hazelcast>
    <network>
