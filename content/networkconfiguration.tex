...
todo: make reference to tcp/ip cluster if testing locally and multicast isn't working.
...
Apart from multicast and the tcp/ip-cluster join mechanisms, there a third mechanism: Amazon EC2. It only works for a single region; so machines over different regions can't discover each other. This could be an issue. todo: how are the members discovered? I guess a aws call is done to read all members (filtered based on some property). I guess that a tcp/ip cluster is used behind the screens. This also shows the power of the programmatic api; because you can easily retrieve the member information from any cloud (e.g. using jclouds) and build up the TcpIpConfig on the fly based on that information. So you don't need Hazelcast support to integrate with other cloud providers.
...
The aws tag accepts an attribute called "conn-timeout-seconds". The default value is 5. Increasing this value is recommended if you have many IP's listed and members can not properly build up the cluster.

The aws section also has a few optional arguments:
\begin{enumerate}
...
Normally Hazelcast prevents the master and the backup partitions to be stored on the same JVM to guarantee high availability and Hazelcast will evenly distibutes the partitions (the master and backups) among the machines. But in some cases you want to control this behavior e.g. when the machines have different capacity or when multiple JVM's run on the same machine and you want to prevent that master and backup are being stored on the same machine because this prevents high availability.

Luckily Hazelcast provides a solution for that in the form of partition groups; in single group a master and backup are not allowed. The simplest configuration is when each machine has its own partition group:
\begin{lstlisting}[language=xml]
<partition-group enabled="true" group-type="HOST_AWARE" />
\end{lstlisting}
Using this configuration if multiple JVM's are started on the same machine they will not share the master AND the backup. 

todo: the partitiongroup is only to prevent master/backup issues. Or can you also control the number of partitions each group ges
...
When you enable partition grouping, Hazelcast presents two choices to configure partition groups at the moments. First one is to group members automatically using IP addresses of members, so members sharing same network interface will be grouped together. 
...
Hazelcast used to have support for asymmetric encryption, but due its complex setup, this feature has been removed from Hazelcast 3.0. Currently there is no support for encryption between a native client and a cluster member.
